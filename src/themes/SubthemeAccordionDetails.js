...
function pluralize(number, a, b, c) {
  ...
  if (2 <= number && number <= 4)
    return b;
  if (number > 100) return pluralize(number % 100)
        ...
        {subthemes.map(it => (
          <li key={it.id}>
            <Typography variant="body2">
              <Link
                to={join(url, props.id, "subthemes", it.id)}>{it.name}</Link> - {it["task_count"]} {pluralize(it["task_count"], "задание", "задания", "заданий")}
            </Typography>
          </li>
        ))}
      </ul>
